...
#WHO AQG 2021 Richtwert für den Tagesmittelwert für PM2,5
PM25EU_AQI_BREAKPOINTS = (
    (0.0, 12.0, 0, 50),
    (12.1, 35.4, 51, 100),
    (35.5, 55.4, 101, 150),
    (55.5, 150.4, 151, 200),
...
def _eu_aqi_from_pm25(pm25_value):
    """Calculate EU AQI from PM2.5 concentration using EPA breakpoints."""
    pm25 = _truncate_pm25_for_aqi(pm25_value)
    if pm25 is None:
        return None

    for c_lo, c_hi, i_lo, i_hi in PM25EU_AQI_BREAKPOINTS:
        if c_lo <= pm25 <= c_hi:
            return int(round(((i_hi - i_lo) / (c_hi - c_lo)) * (pm25 - c_lo) + i_lo))
    return None


    if pm25 > PM25EU_AQI_BREAKPOINTS[-1][1]:
        c_lo, c_hi, i_lo, i_hi = PM25EU_AQI_BREAKPOINTS[-1]
        return int(round(((i_hi - i_lo) / (c_hi - c_lo)) * (pm25 - c_lo) + i_lo))
...
class getData(SearchList):
    ...
    RECORDS_OPTIONAL_OBSERVATIONS = ("appTemp", "windrun", "UV", "radiation", "sunshineDur", "rainDur")
    ...
    def get_extension_list(self, timespan, db_lookup):
        ...
        highcharts_local = extras_dict.get("highcharts_local", None)
        if highcharts_local is None or highcharts_local == "0":
            highcharts_local = False
        else:
            highcharts_local = True
        ...
        rainDur_unit = converter.group_unit_dict["group_deltatime"]
        skin_rainDur_unit = self.generator.converter.group_unit_dict["group_deltatime"]
        rainDur_round = skin_dict["Units"]["StringFormats"].get(
            skin_rainDur_unit, "%.2f"
        )
        ...
        if driver == "weedb.sqlite":
            ...
            """
            at_rainDur_highest_year_sql = """
                SELECT strftime('%Y', datetime(dateTime, 'unixepoch', 'localtime')) AS year, SUM(sum) AS total
                FROM archive_day_rainDur
                GROUP BY year ORDER BY total DESC LIMIT 1;
            """
        ...
        if at_rain_highest_year_query is not None and len(at_rain_highest_year_query) >= 2:
            at_rain_highest_year_raw = [
                at_rain_highest_year_query[0],
                weewx.units.convert(
                    (at_rain_highest_year_query[1], rain_unit, "group_rain"), "mm"
                )[0],
            ]
        else:
            at_rain_highest_year_raw = ["N/A", 0.0]

        if records_obs_available["rainDur"]:
          # All time rainiestDur year
          at_rainDur_highest_year_query = wx_manager.getSql(at_rainDur_highest_year_sql)
          if at_rainDur_highest_year_query is not None and len(at_rainDur_highest_year_query) >= 2:
            at_rainDur_highest_year_tuple = (
                at_rainDur_highest_year_query[1],
                rainDur_unit,
                "group_deltatime",
            )
            at_rainDur_highest_year_converted = (
                rainDur_round % self.generator.converter.convert(at_rainDur_highest_year_tuple)[0]
            )
            at_rainDur_highest_year = [
                at_rainDur_highest_year_query[0],
                locale.format_string("%g", float(at_rainDur_highest_year_converted)),
            ]
          else:
            at_rainDur_highest_year = ["N/A", 0.0]
            ...
            if obs=='radiation':
                ...
                if maxSolarRad_val is not None:
                    ...
                    obs_maxSolarRad_output += (
                        "<span class='maxSolarRad'>î %s</span>" % maxSolarRad_val
                    )
        ...
        # Compass color 0 .. 6
        # ==============================================================================
        compass_color = "#808080"

        compass_color0 = extras_dict.get("beauford0", "#38C8F6")
        compass_color1 = extras_dict.get("beauford1", "#1278C8")
        compass_color2 = extras_dict.get("beauford2", "#71BC3C")
        compass_color3 = extras_dict.get("beauford3", "#FFAE00")
        compass_color4 = extras_dict.get("beauford4", "#FF7F00")
        compass_color5 = extras_dict.get("beauford5", "#FF4500")
        compass_color6 = extras_dict.get("beauford6", "#9F00C5")
        ...
        search_list_extension = {
            ...
            "at_rainDur_highest_year": at_rainDur_highest_year,
            ...
            "records_has_rain_dur": records_obs_available["rainDur"],
            ...
            **noaa_search_list_values,
            ...
            "compass_color0": compass_color0,
            "compass_color1": compass_color1,
            "compass_color2": compass_color2,
            "compass_color3": compass_color3,
            "compass_color4": compass_color4,
            "compass_color5": compass_color5,
            "compass_color6": compass_color6,
    ...
    def get_observation_data(
        ...
        if observation == "weatherRange":
            ...
            try:
                ...
                log.error(
                    f"Error trying to use database binding {binding} to min chart observation {obs_lookup}. "
                    f"Error was: {e}."
                )
            ...
            try:
                ...
                log.error(
                    f"Error trying to use database binding {binding} to max chart observation {obs_lookup}. "
                    f"Error was: {e}."
                )
            ...
            try:
                ...
                log.error(
                    f"Error trying to use database binding {binding} to avg chart observation {obs_lookup}. "
                    f"Error was: {e}."
                )
        ...
        try:
            ...
            log.error(
                f"Error trying to use database binding {binding} to standard chart observation {obs_lookup}. Error was: {e}."
            )
            return []
