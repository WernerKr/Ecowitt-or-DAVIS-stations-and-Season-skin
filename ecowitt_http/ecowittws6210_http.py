...
class SdMapper(FieldMapper):
    ...
    def map_data(self, rec, unit_system=None):
        ...
        if self.field_map is not None and len(self.field_map) > 0:
            ...
            #log.info("self.field_map %s", self.field_map) 
            #log.info("rec.keys %s", rec.keys()) 
            mapped_rec = {}
            ...
            for field in rec.keys():
                ...
                try:
                    ...
                    #log.info("dest_field %s, clean_key %s", dest_field, clean_key) 
                    if clean_key == 'Thunder time':
                       ...
                       continue
                    #elif 'Pm2.5 CH1' in field:
                    #   mapped_rec['ch_pm25.1.PM25'] = float(rec[field])
                    #   continue
                    #elif 'Pm2.5 CH2' in field:
                    #   mapped_rec['ch_pm25.2.PM25'] = float(rec[field])
                    #   continue
                    #elif 'Pm2.5 CH3' in field:
                    #   mapped_rec['ch_pm25.3.PM25'] = float(rec[field])
                    #   continue
                    #elif 'Pm2.5 CH4' in field:
                    #   mapped_rec['ch_pm25.4.PM25'] = float(rec[field])
                    #   continue
...
class EcowittSensors:
    ...
    batt_volt = ('wh68', 'wh51', 'wh54', 'wn34', 'wn38', 'wn35', 'ws80', 'ws85', 'ws90')
