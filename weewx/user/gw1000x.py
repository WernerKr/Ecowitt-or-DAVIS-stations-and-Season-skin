...
Version: 0.6.4kw                                    Date: 04 April 2026
...
class Gateway(object):
    ...
    battery_field_map = {
        ...
        'wn34_ch9_batt': 'wn34_ch9_batt',
        'wn34_ch10_batt': 'wn34_ch10_batt',
        'wn34_ch11_batt': 'wn34_ch11_batt',
        'wn34_ch12_batt': 'wn34_ch12_batt',
        'wn34_ch13_batt': 'wn34_ch13_batt',
        'wn34_ch14_batt': 'wn34_ch14_batt',
        'wn34_ch15_batt': 'wn34_ch15_batt',
        'wn34_ch16_batt': 'wn34_ch16_batt',
    ...
    sensor_signal_field_map = {
        ...
        'wn34_ch9_sig': 'wn34_ch9_sig',
        'wn34_ch10_sig': 'wn34_ch10_sig',
        'wn34_ch11_sig': 'wn34_ch11_sig',
        'wn34_ch12_sig': 'wn34_ch12_sig',
        'wn34_ch13_sig': 'wn34_ch13_sig',
        'wn34_ch14_sig': 'wn34_ch14_sig',
        'wn34_ch15_sig': 'wn34_ch15_sig',
        'wn34_ch16_sig': 'wn34_ch16_sig',
    ...
    def delta_rain(rain, last_rain, descriptor='rain'):
        ...
        if rain < last_rain:
            ...
            if (rain) > 2:										#KW
               loginf("%s too high rain detected: new=%s last=%s" % (descriptor, rain, last_rain))
               return 0
            return rain
...
class GatewayService(weewx.engine.StdService, Gateway):
    ...
    def __init__(self, engine, config_dict):
        ...
        loginf("GatewayService: version is %s - Device is %s" % (DRIVER_VERSION,self.collector.firmware_version))
    ...
    def new_loop_packet(self, event):
        ...
        if self.cached_sensor_data is not None:
            ...
            ##self.calculate_rain(self.cached_sensor_data)		# Werner removed

            # get the lightning strike count this period from total
            ## self.calculate_lightning_count(self.cached_sensor_data)	# Werner removed
...
class GatewayDriver(weewx.drivers.AbstractDevice, Gateway):
    ...
    def __init__(self, **stn_dict):
        ...
        # log our version number
        loginf("GatewayDriver: version is %s - Hardware is %s" % (DRIVER_VERSION,self.collector.firmware_version))
...
class GatewayCollector(Collector):
    ...
    sensor_ids = {
        ...
        b'\x03': {'name': 'wh40', 'long_name': 'WH40', 'batt_fn': 'batt_volt_wh40'},		##kw
    ...
    class Station(object):
        ...
        header = b'\xff\xff'
        # known device models
        known_models = ('GW1000', 'GW1100', 'GW1200', 'GW2000', 'GW3000', 'WS6210',
    ...
    class Parser(object):
        ...
        rain_data_struct = {
            ...
            b'\x82': ('decode_rain', 2, 'p_rainhour'),
        ...
        def parse_get_pm25_offset(response):
            ...
            4       size            byte
            5       channel 1       byte            fixed 00
            6-7     pm25 offset     signed short    -200 to +200 in tenths Âµg/mÂ³
                                                    (-20.0 to +20.0)
            ....
            14      channel 1       byte            fixed 00
            15-16   pm25 offset     signed short    -200 to +200 in tenths Âµg/mÂ³
                                                   (-20.0 to +20.0)
        ...
        def parse_get_co2_offset(response):
            ...
            4       size            byte
            5-6     co2 offset      signed short    -600 to +10000 in tenths Âµg/mÂ³
            7-8     pm25 offset     signed short    -200 to +200 in tenths Âµg/mÂ³
                                                   (-20.0 to +20.0)
            9-10    pm10 offset     signed short    -200 to +200 in tenths Âµg/mÂ³
                                                   (-20.0 to +20.0)
            17      checksum        byte            LSB of the sum of the
                                                    command, size and data
                                                    bytes
        ...
        def decode_rain_reset(data, fields=None):
            ...
            if len(data) == 3:
                results = dict()
                results['rain_day_reset'] = struct.unpack("B", data[0:1])[0]
                results['rain_week_reset'] = struct.unpack("B", data[1:2])[0]
                results['rain_annual_reset'] = struct.unpack("B", data[2:3])[0]
                return results
            return {}
                ...
                if GatewayCollector.sensor_ids[address].get('name') in GatewayCollector.no_low:
                    ...
                    if batt_fn == 'batt_binary':
                        ...
                        if value <= 1:
                            ...
                            return 'Unknown'
                    elif batt_fn == 'batt_volt' or batt_fn == 'batt_volt_tenth' or batt_fn == 'batt_volt_wh40':
...
class DirectGateway(object):
    ...
    gateway_metric_units = weeutil.weeutil.ListOfDicts({
    ...
    gateway_metricwx_units = weeutil.weeutil.ListOfDicts({
        "group_altitude": "meter",
        "group_distance": "km",
        "group_length": "cm",
        "group_pressure": "hPa",
    ...
    def system_params(self):
        ...
        try:
            ...
            print("Device/Firmware: %s" % collector.firmware_version)
    ...
    def get_rain_data(self):
        ...
        try:
            ...
            print()
            #print("Interrogating %s at %s:%d" % (collector.station.model,
            print("Interrogating %s (Device %s) at %s:%d" % (collector.station.model,collector.firmware_version,
                                                 collector.station.ip_address.decode(),
                                                 collector.station.port))
    ...
    def get_all_rain_data(self):
        ...
        traditional = ['t_rainrate', 't_rainevent', 't_rainday',
                       't_rainweek', 't_rainmonth', 't_rainyear', 'raingain']
        piezo = ['p_rainrate', 'p_event', 'p_day', 'p_hour', 'p_week', 'p_month', 'p_year',
                 'gain0', 'gain1', 'gain2', 'gain3', 'gain4', 'gain5']
        reset = ['rain_day_reset', 'rain_week_reset', 'rain_annual_reset']
        ...
        try:
            ...
            if any(field in rain_data for field in traditional):
                ...
                _data = rain_data.get('raingain', None)
                _data_str = "%.1f " % (_data) if _data is not None else "---"
            ...
            if any(field in rain_data for field in piezo):
                ...
                _data_str = "%.1fmm (%.1fin)" % (_data, _data / 25.4) if _data is not None else "---mm (---in)"
                print("%30s: %s" % ('Event rain', _data_str))
                _data = rain_data.get('p_rainhour', None)
                _data_str = "%.1fmm (%.1fin)" % (_data, _data / 25.4) if _data is not None else "---mm (---in)"
                print("%30s: %s" % ('Hourly rain', _data_str))
                ...
                _data_str = "%.1fmm (%.1fin)" % (_data, _data / 25.4) if _data is not None else "---mm (---in)"
                print("%30s: %s" % ('Yearly rain', _data_str))
                print("%30s: %.2f (%s)" % ('Rain1 gain', rain_data.get('gain0', '--'), '< 4mm/h'))
                print("%30s: %.2f (%s)" % ('Rain2 gain', rain_data.get('gain1', '--'), '< 10mm/h'))
                print("%30s: %.2f (%s)" % ('Rain3 gain', rain_data.get('gain2', '--'), '< 30mm/h'))
                print("%30s: %.2f (%s)" % ('Rain4 gain', rain_data.get('gain3', '--'), '< 60mm/h'))
                print("%30s: %.2f (%s)" % ('Rain5 gain', rain_data.get('gain4', '--'), '> 60mm/h'))
            else:
                print("    No piezo rain data available")
            print()
            if any(field in rain_data for field in reset):
                print("    Rainfall reset time data:")
                print("%30s: 0%d:00" % ('Daily rainfall reset time', rain_data.get('rain_day_reset', '-----')))
                _data = rain_data.get('rain_week_reset', None)
                _data_str = "%s" % calendar.day_name[(_data + 6) % 7] if _data is not None else "-----"
                print("%30s: %s" % ('Weekly rainfall reset time', _data_str))
                _data = rain_data.get('rain_annual_reset', None)
                _data_str = "%s" % calendar.month_name[_data + 1] if _data is not None else "-----"
                print("%30s: %s" % ('Annual rainfall reset time', calendar.month_name[rain_data['rain_annual_reset'] + 1]))
            else:
    ...
    def live_data(self):
        ...
        try:
            ...
            if self.opts.units.lower() == 'us':
                c = weewx.units.Converter(group_unit_dict=DirectGateway.gateway_us_units)
            elif self.opts.units.lower() == 'metricwx':
                c = weewx.units.Converter(group_unit_dict=DirectGateway.gateway_metricwx_units)
            else:
                c = weewx.units.Converter(group_unit_dict=DirectGateway.gateway_metric_units)
#            if self.opts.units.lower() == 'us':
...
#            c = weewx.units.StdUnitConverters[_unit_system]
            ...
            gw_unit_format_dict['km_per_hour'] = '%.1f'						##KW
            gw_unit_format_dict['km_per_hour2'] = '%.2f'
            gw_unit_format_dict['mile_per_hour'] = '%.1f'
            gw_unit_format_dict['mile_per_hour2'] = '%.2f'
            gw_unit_format_dict['knot'] = '%.1f'
            gw_unit_format_dict['knot2'] = '%.2f'
            gw_unit_format_dict['uv_index'] = '%.2f'
